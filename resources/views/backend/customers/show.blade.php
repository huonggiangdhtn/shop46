    ...
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <div class="lg:flex intro-y box py-5 px-5">
              <div class='relative'> 
                <div class= "mt-3">
                    <label class="font-medium"> Khách hàng: </label>
                    {{$customer->full_name}}
                </div>
                <div class= "mt-3">
                    <label class="font-medium"> Tổng công nợ: </label>
                    <span class="{{$customer->budget < 0?'text-danger':'text-success'}}">{{Number_format($customer->budget,0,'.',',')}}</span>
                    <br/><span class="form-help"> (-) khách hàng nợ tiền , (+) cửa hàng nợ tiền </span>
                </div>
              </div>
              <div class="mt-3 lg:w-auto   lg:mt-0 ml-auto">
                <a href="{{route('customer.received',$customer->id)}}" class="btn btn-primary shadow-md mr-2 primary-btn lg:w-auto   lg:mt-0 ml-auto" > trả tiền khách hàng </a>
                <a href="{{route('customer.paid',$customer->id)}}" class="btn btn-primary shadow-md mr-2 primary-btn lg:w-auto   lg:mt-0 ml-auto" > nhận tiền khách hàng </a>
                <a href="{{route('customer.balance',$customer->id)}}" class="btn btn-primary shadow-md mr-2 primary-btn lg:w-auto   lg:mt-0 ml-auto" > khấu trừ tiền công nợ </a>
                ...
                <div class="container {{$sp->operation==-1?'right':'left'}}"style="float:{{$sp->operation==-1?'right':'left'}}">
                    ...
                    <div class="flex items-center">
                        ...
                        <?php
                          ...
                          if($sp->doc_type == "fi")
                          {
                              $str_route = route('suptransaction.show',$sp->doc_id);
                          }
                        ...
                        <?php
                                ...
                                if($sp->doc_type == 'wi')
                                {
                                    if($sp->operation == 1)
                                    {
                                        ...
                                        .'"> phiếu nhập: '.$sp->doc_id .'</a> : '.  Number_format($sp->amount,0,'.',',');
                                    ...
                                    {
                                      echo '<a class="font-medium text-warning" href ="'.route('warehousein.show',$sp->doc_id )
                                ...
                                if($sp->doc_type =='wo')
                                {
                                    if($sp->operation == -1)
                                    {
                                        ...
                                        .'"> phiếu xuất: '.$sp->doc_id .'</a> : '.  Number_format($sp->amount,0,'.',',');
                                ...
                                if($sp->doc_type == 'fi')
                                {
                                    echo '<a class="font-medium" href ="'.route('banktrans.show',$sp->doc_id)
                                    .'"> Mã giao dịch tài khoản: '.$sp->doc_id.' </a>';
                                    $bt = \App\Models\Banktransaction::where('id',$sp->doc_id)->first();
                                    echo ' với số tiền: '.Number_format($bt->total,0,'.',',');
                                
                                }
                            ?>
                           
                        <br/>
                        công nợ: <span class="{{$sp->total <= 0?'text-danger':''}}">{{Number_format($sp->total,0,'.',',')}}<span>
                    </p>
